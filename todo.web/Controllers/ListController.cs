...
namespace todo.web.Controllers
{
    public class ListController : BaseController
    {
        ...
        [HttpGet("/{culture:regex(en)}/lists/{id}/items/{itemId}/delete")]
        [HttpGet("/{culture:regex(fr)}/listes/{id}/elements/{itemId}/supprimer")]
        public async Task<IActionResult> DeleteItem(int id, int itemId)
        {
            var result = await _client.DeleteItem(id, itemId);
            if (!result.IsSuccessStatusCode)
                throw new ResultException($"Unable to delete item in list {id} for {UserName}", result);

            SetSuccessMessage();
            return RedirectToAction(nameof(GetList), new { id = id});
        }
        ...
        public async Task<IActionResult> DeleteList(int id)
        {
           var result = await _client.DeleteList(id);
            if (!result.IsSuccessStatusCode)
                throw new ResultException($"Unable to delete list for {UserName}", result);
